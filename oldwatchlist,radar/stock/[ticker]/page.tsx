...
function generateCandles(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const target = startPrice + (endPrice - startPrice) * progress + (rng(i * 3) - 0.5) * volatility * startPrice * 2;
...
function CandlestickChart({ candles, isPositive, width = 700, height = 220 }: {
  candles: CandleData[];
  isPositive: boolean;
  width?: number;
  height?: number;
  ...
  const linePoints = candles.map((c, i) => `${pad.left + i * gap + gap / 2},${toY(c.close)}`);
  const linePath = "M " + linePoints.join(" L ");
  const areaPath = linePath + ` L ${pad.left + (candles.length - 1) * gap + gap / 2},${pad.top + chartH} L ${pad.left + gap / 2},${pad.top + chartH} Z`;
  const accentColor = isPositive ? bullColor : bearColor;

  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" height={height} aria-label="Price chart" role="img">
      <defs>
      ...
      {candles.map((c, i) => {
        ...
        return (
          <g key={i}>
            <line x1={x} y1={toY(c.high)} x2={x} y2={toY(c.low)} stroke={color} strokeWidth={0.9} opacity={0.6} />
            <rect x={x - candleW / 2} y={bodyTop} width={candleW} height={bodyH} fill={color} opacity={0.85} rx={0.5} />
          </g>
        );
      })}
      <path d={linePath} fill="none" stroke={accentColor} strokeWidth={1.5} opacity={0.5} />
    </svg>
  );
}
...
function StatCard({ label, value, sub, color }: { label: string; value: string; sub?: string; color?: string }) {
  return (
    <div className="bg-white dark:bg-white/[0.04] border border-gray-100 dark:border-white/[0.06] rounded-xl p-4">
      ...
      <span className={cn("font-outfit font-bold text-[18px] block", color ?? "text-text-primary-light dark:text-text-primary-dark")}>
        {value}
      </span>
      {sub && (
...
export default function StockDetailPage() {
  ...
  const candles = useMemo(() => {
    if (period === "Day") return generateCandles(48, stock.currentPrice * 0.98, stock.currentPrice, 0.004);
        ...
        <div className="flex items-start justify-between gap-6 flex-wrap">
          ...
          <div className="flex items-center gap-4">
            <div
              ...
              style={{ background: `linear-gradient(135deg, ${(TICKER_GRADIENTS[ticker] ?? ["#7C7C7C", "#3C3C3C"])[0]}, ${(TICKER_GRADIENTS[ticker] ?? ["#7C7C7C", "#3C3C3C"])[1]})` }}
            >
              <img
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {stock.hasWarning && <AlertTriangle size={16} className="text-warning" />}
              </div>
          ...
          <div className="bg-white dark:bg-white/[0.03] rounded-2xl border border-gray-200 dark:border-white/[0.06] shadow-sm overflow-hidden">
            ...
            <div className="flex items-center gap-1 px-4 pt-4 pb-2">
              ...
              <div className="ml-auto flex items-center gap-1.5 text-text-secondary-light dark:text-text-secondary-dark">
                <Volume2 size={13} />
                <span className="font-inter text-[12px] font-semibold">{volume}</span>
              </div>
            </div>
            <div className="px-4 pb-4">
              <AnimatePresence mode="wait">
                ...
                >
                  <CandlestickChart candles={candles} isPositive={isPositive} height={220} />
                </motion.div>
              </AnimatePresence>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="sm:col-span-2 bg-white dark:bg-white/[0.03] rounded-2xl border border-primary/10 dark:border-white/[0.08] shadow-sm p-5">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-7 h-7 rounded-lg bg-primary/10 dark:bg-white/10 flex items-center justify-center">
                  <Sparkles size={14} className="text-primary dark:text-white" />
                </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                <Sparkles size={20} />
                Ask Raven AI
                <span className="font-normal text-[11px] opacity-70">about {ticker}</span>
              </button>
              <button
