      ...
      >
        <div className="flex items-center justify-between mb-5">
          <h3 className="font-outfit font-bold text-[18px] text-primary dark:text-white">
            {(dict as any).radar?.filter ?? 'Filters'}
          </h3>
          <button
            onClick={onClose}
...
function DesktopAiRadar({ state }: { state: RadarPageState }) {
  ...
  const buyCnt = RADAR_ITEMS.filter((i) => !i.isClosed && i.action === 'buy')
    .length
    ...
    <div className="hidden lg:flex flex-col h-full market-pattern">
      <div className="sticky top-0 bg-white/95 dark:bg-[#1a202c]/95 backdrop-blur-md z-10 border-b border-gray-200 dark:border-gray-800 px-4 py-4 h-[64px] flex justify-between items-center">
        <div className={cn('flex items-center gap-2', isRTL && 'flex-row-reverse')}>
          <h2 className="text-[16px] font-bold">{dict.radar.title}</h2>
          <div
            className={cn(
        ...
        <div className="flex items-center gap-2 relative">
            ...
            >
              <Filter size={11} />
              {(dict as any).radar?.filter ?? 'Filter'}
            </button>
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        <div className="p-4 flex flex-col gap-4">
          ...
          {[{ label: dict.radar.buy, count: buyCnt, color: 'bg-green-500' }, { label: dict.radar.watch, count: watchCnt, color: 'bg-slate-400' }, { label: dict.radar.avoid, count: avoidCnt, color: 'bg-red-500' }].map(
            (b) => (
              <div
                key={b.label}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white dark:bg-white/[0.04] border border-gray-200 dark:border-white/10"
              >
                ...
                <span className="font-inter font-bold text-[10px] text-text-primary-light dark:text-text-primary-dark tabular-nums">
                  {b.count}
                </span>
              </div>
            )
...
export default function RadarPanel({ compact }: { compact?: boolean }) {
  ...
  return (
    <div className="h-full w-full overflow-y-auto">
      ...
      {compact && <MobileAiRadar state={sharedState} compact />}
    </div>
  )
}
